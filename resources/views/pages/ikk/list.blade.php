...
@section('subhead')
    <title>Indikator Kegiatan Kegiatan</title>
@endsection

@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Indikator Kinerja Kegiatan {{$user->eselon}}</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <a class="btn btn-primary shadow-md mr-2" href="{{ url('input-form-ikk') }}">Tambah Realisasi</a>
        </div>
    </div>
    ...
    <div class="intro-y box p-5 mt-5">
        <div class="flex flex-col sm:flex-row sm:items-end xl:items-start">
            <form action="{{ route('list-ikk') }}" method="GET"  id="tabulator-html-filter-form" class="xl:flex sm:mr-auto" >
            ...
            <table class="table">
                ...
                <tbody>
                    @foreach($data as $data)
                        @if (count($data->ikk) > 1)
                        <tr>
                            <td class="border-b dark:border-dark-5" rowspan="{{count($data->ikk) + 1}}" >SS-{{ $data->nomor }}<br><p class="text-justify">{{$data->deskripsi}}</p></td> 
                        </tr>
                        @foreach($data->ikk as $ikk)
                            <tr>
                                <td class="border-b dark:border-dark-5">{{$ikk->deskripsi}}</td>
                                <td class="border-b dark:border-dark-5">{{$ikk->target}}</td>
                                <td class="border-b dark:border-dark-5">{{$ikk->capaian}}</td>
                                <td class="border-b dark:border-dark-5">{{$ikk->analisa}}</td>
                                <td class="border-b dark:border-dark-5"><pre>{{$ikk->kegiatan}}</pre></td>
                                <td class="border-b dark:border-dark-5">{{$ikk->kendala_hambatan}}</td>
                                @if ($ikk->capaian_id != '')
                                <td class="border-b dark:border-dark-5">
                                    ...
                                    <a class="btn btn-sm  btn-danger mr-1 mb-2" href="{{ url('ikk/'.$data->ikk[0]->id.'/delete') }}">
                        ...
                        <tr>
                            ...
                            <td class="border-b dark:border-dark-5">{{$data->ikk[0]->deskripsi}}</td>
                            <td class="border-b dark:border-dark-5">{{$data->ikk[0]->target}}</td>
                            <td class="border-b dark:border-dark-5">{{$data->ikk[0]->capaian}}</td>
                            <td class="border-b dark:border-dark-5">{{$data->ikk[0]->analisa}}</td>
                            <td class="border-b dark:border-dark-5"><pre>{{$data->ikk[0]->kegiatan}}</pre></td>
                            <td class="border-b dark:border-dark-5">{{$data->ikk[0]->kendala_hambatan}}</td>
                                @if ($data->ikk[0]->capaian_id != '')
                                <td class="border-b dark:border-dark-5">
                                    <div class="box mt-5">
                                    {{-- <a class="btn btn-sm btn-warning mr-1 mb-2" href="javascript:;">
                                        <i data-feather="edit" class="w-3 h-3"></i>
