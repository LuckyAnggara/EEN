...
@section('subcontent')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Input Realisasi Indikator {{$user->eselon}}</h2>
    </div>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <form action="{{route('store-ikk')}}" method="post" class="intro-y col-span-12 lg:col-span-6">
            {{ csrf_field() }}
            <div >
                <!-- BEGIN: Form Layout -->
                <div class="intro-y box p-5">
                    ...
                    <div class="mt-3">
                        <label for="crud-form-2" class="form-label">Pelaksana</label>
                        <input id="pelaksana" name="pelaksana" type="text" class="form-control w-full" placeholder="pelaksana" value="{{$user->eselon}}" disabled>
                    </div>
                    <div class="mt-3">
                        <label for="crud-form-2" class="form-label">Indikator Kegiatan Utama</label>
                        <select data-placeholder="Indikator Kegiatan Utama" class="tom-select w-full" name="ikk" id="ikk">
                            @foreach ($ikk as $item)
                    ...
                    <div class="mt-3">
                        ...
                        <input id="target" type="text" class="form-control w-full" placeholder="Target Indikator Kinerja Utama" value="{{$ikk[0]->target}}" disabled>
                    </div>
                    <div class="mt-3">
