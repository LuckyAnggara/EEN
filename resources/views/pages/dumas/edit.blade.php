...
@section('subcontent')
<form action="{{ route('update-dumas') }}" method="post" >  
    {{ csrf_field() }}
    <input name="bulan"  type="hidden" value="{{$bulan}}"/>
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Edit Pengelolaan Daftar Pengaduan Masyarakat Bulan {{$bulanString}}</h2>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="border-b dark:border-dark-5">
                            <input name="proses_kanwil[{{$data->id}}]" type="text" class="form-control sm:mt-0" value="{{$data->proses_kanwil}}">
                        </td>
                        <td class="border-b dark:border-dark-5">
                            <input name="jumlah_selesai[{{$data->id}}]" type="text" class="form-control sm:mt-0" value="{{$data->jumlah_selesai}}">
                        </td>
                    </tr>
                    @endforeach
                </tbody>
