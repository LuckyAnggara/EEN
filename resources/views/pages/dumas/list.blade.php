...
@section('subcontent')
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Pengelolaan Daftar Pengaduan Masyarakat</h2>
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            <a class="btn btn-primary shadow-md mr-2" href="{{ url('input-form-dumas') }}">Tambah Data</a>
        </div>
    </div>
    ...
    <div class="intro-y alert alert-warning alert-dismissible show flex items-center mb-2 mt-2" role="alert">
        <i data-feather="alert-octagon" class="w-6 h-6 mr-2"></i> {{$warning}} <a href="{{ url('input-form-dumas') }}"><strong>&nbsp<u>Tambah Data</u></strong></a>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close">
            <i data-feather="x" class="w-4 h-4"></i>
        </button>
    </div>
    ...
    <div class="intro-y box p-5 mt-5">
        <div class="flex flex-col sm:flex-row sm:items-end xl:items-start">
            <form action="{{ route('list-dumas') }}" method="GET"  id="tabulator-html-filter-form" class="xl:flex sm:mr-auto" >
            ...
            <div class="flex mt-5 sm:mt-0">
                <button class="btn btn-outline-secondary w-1/2 sm:w-auto mr-2">
                    <a href="{{ route('edit-form-dumas', ['bulan' => $params->input('bulan') == null ? date('m') : $params->input('bulan')]) }}">
                        ...
                        <div class="modal-content">
                            <div class="modal-body p-0">
                                <div class="p-5 text-center">
                                    ...
                                    <div class="text-gray-600 mt-2">Data Pengaduan Masyarakat bulan {{$bulanString}} akan di <strong>Hapus</strong>!</div>
                                </div>
                                <form action="{{ route('delete-dumas', ['bulan' => $params->input('bulan') == null ? date('m') : $params->input('bulan')])}}" method="post">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="border-b dark:border-dark-5">{{!empty($data->dumas->berkas_masuk) ? $data->dumas->berkas_masuk : '-' }}</td>
                        <td class="border-b dark:border-dark-5">{{!empty($data->dumas->proses_auditor) ? $data->dumas->proses_auditor : '-' }}</td>
                        <td class="border-b dark:border-dark-5">{{!empty($data->dumas->proses_irjen) ? $data->dumas->proses_irjen : '-' }}</td>
                        <td class="border-b dark:border-dark-5">{{!empty($data->dumas->proses_menteri) ? $data->dumas->proses_menteri : '-' }}</td>
                        <td class="border-b dark:border-dark-5">{{!empty($data->dumas->proses_kanwil) ? $data->dumas->proses_kanwil : '-' }}</td>
                        <td class="border-b dark:border-dark-5">{{!empty($data->dumas->jumlah_selesai) ? $data->dumas->jumlah_selesai : '-' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
